...
mysql -u root -p"$MYSQL_ROOT_PASS" -e "CREATE DATABASE IF NOT EXISTS $MYSQL_PAYMENTS_DB;"
...
mysql -u root -p"$MYSQL_ROOT_PASS" -e "GRANT ALL PRIVILEGES ON $MYSQL_USER_DB.* TO '$MYSQL_USER'@'%';"
mysql -u root -p"$MYSQL_ROOT_PASS" -e "GRANT ALL PRIVILEGES ON $MYSQL_PAYMENTS_DB.* TO '$MYSQL_USER'@'%';"
